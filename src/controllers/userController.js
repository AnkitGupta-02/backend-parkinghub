...
exports.createUser = async (req, res) => {
  ...
  if (!userdata) return res.status(400).send("req.body cannot be empty");

  const user = await createUser(userdata);
  if (!user) return res.status(400).send("user creation failed");

  const emailSent = await verifyEmailSent(user);
  if (!emailSent) return res.status(500).send("email verfication failed");
